...
// Real root filesystem usage percent via df (Linux only; 0 elsewhere)
...
// Check whether a binary exists on PATH, and best-effort read its version line
...
// Real installed-state detection for common CLI packages and language runtimes, by probing PATH
...
// Real active systemd service units (Linux only; empty elsewhere since systemd doesn't exist there)
    ...
    return output
      ...
      .map((line) => {
        ...
        return {
          ...
          subState: sub,
          // list-units doesn't expose boot-time enablement without a per-unit `systemctl is-enabled`
          // call; approximated from current active state rather than fabricated.
...
const server = http.createServer(async (req, res) => {
  ...
  if (pathname === '/api/v1/health' || pathname === '/health') {
    ...
    // Gated: this can return file contents, so requires the same agent token as the other mutating endpoints
    if (!isAuthorized(req)) {
      res.writeHead(401);
    ...
    // Execute real shell command on host Linux VPS -- requires a valid agent token
    if (!isAuthorized(req)) {
      res.writeHead(401);
      res.end(JSON.stringify({ error: 'Unauthorized: missing or invalid agent token' }));
      return;
    ...
    // Real `apt-get install` for a package/runtime, gated by token + a strict name whitelist
    if (!isAuthorized(req)) {
      res.writeHead(401);
      res.end(JSON.stringify({ error: 'Unauthorized: missing or invalid agent token' }));
      return;
    ...
    // Execute docker container actions via execFile (no shell) with a whitelisted action + id pattern
    if (!isAuthorized(req)) {
      res.writeHead(401);
      res.end(JSON.stringify({ error: 'Unauthorized: missing or invalid agent token' }));
      return;
    ...
    // Execute systemctl service control via execFile (no shell) with a whitelisted action + unit name pattern
    if (!isAuthorized(req)) {
      res.writeHead(401);
      res.end(JSON.stringify({ error: 'Unauthorized: missing or invalid agent token' }));
      return;
