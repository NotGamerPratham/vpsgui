...
export function CommandPalette() {
  ...
  const filteredNodes = nodes.filter((n) => n.name.toLowerCase().includes(query.toLowerCase()) || n.network.publicIp.includes(query));
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-20 px-4">
      <div className="fixed inset-0 bg-black/75 backdrop-blur-md transition-opacity" onClick={() => setCommandPaletteOpen(false)} />

      <div className="relative z-50 w-full max-w-2xl rounded-xl border border-border bg-card shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-150">
        {/* Search Input Bar */}
                ...
                {filteredNodes.map((node) => (
                  ...
                  >
                    <div className="flex items-center">
                      ...
                      <span className="ml-2 font-mono text-[10px] text-muted-foreground">({node.network.publicIp})</span>
                    </div>
